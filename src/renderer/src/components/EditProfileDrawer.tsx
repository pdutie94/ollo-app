    ...
    <AnimatePresence>
      ...
      <motion.div key="drawer" variants={slideInRight} initial="initial" animate="animate" exit="exit" className="fixed top-0 right-0 h-full z-50 flex flex-col w-[480px] bg-[var(--card)] border-l border-[var(--border)] font-inter">
        <div className="flex items-center justify-between px-6 py-4 shrink-0 border-b border-[var(--border)]">
          <div>
            <h2 className="text-base font-semibold text-[var(--foreground)]">Sửa Profile</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {activeSection === "Trình duyệt" && (
            <>
              ...
              <FormSelect label="Phiên bản" value="Mới nhất (ổn định)" onChange={() => {}} options={["Mới nhất (ổn định)", "120.0", "119.0", "118.0"]} />
            </>
          )}
          ...
          {activeSection === "Vân tay" && (
            <>
              ...
              <FormSelect label="Múi giờ" value="America/New_York" onChange={() => {}} options={["America/New_York", "Europe/London", "Asia/Tokyo", "Asia/Singapore"]} />
              <FormSelect label="Ngôn ngữ" value="vi-VN" onChange={() => {}} options={["vi-VN", "en-US", "en-GB", "ja-JP"]} />
              <FormSelect label="Độ phân giải" value="1920×1080" onChange={() => {}} options={["1920×1080", "2560×1440", "1366×768", "1280×800"]} />
          ...
          {activeSection === "Khởi động" && (
            <>
              <FormField label="URL khởi động" value="" onChange={() => {}} placeholder="https://example.com" textarea />
              <FormSelect label="Tiện ích" value="Không" onChange={() => {}} options={["Không", "Cookie Manager", "User-Agent Switcher", "Canvas Blocker"]} />
              <div>
        ...
        <div className="flex items-center justify-between px-6 py-4 shrink-0 border-t border-[var(--border)]">
          ...
          <button onClick={handleSave} className="rounded-lg px-4 py-2 bg-[var(--primary)] border-none text-[var(--primary-foreground)] text-[13px] font-medium cursor-pointer">Lưu thay đổi</button>
        </div>
      </motion.div>
    </AnimatePresence>
  );
